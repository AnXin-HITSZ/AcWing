...
> 注意：shell 脚本不一定必须以 `.sh` 结尾，任意结尾（甚至没有后缀）均可。
...
> 以避免带空格文件名造成错误。
>
> 同理，在使用 shell 可执行脚本处理文件夹时，也需要考虑到文件夹名字含有空格的情况：
> * 在远程服务器端创建文件夹（考虑传入的文件参数包含空格的情况）：
>   ```shell
>   # remote_mkdir.sh
>   
>   #! /bin/bash
...
>   ```
>
> 如上述两例所示，在利用 `ssh user@hostname command` 在远程服务器端执行命令时，命令中的文件夹名字需要使用转义后的双引号 `\"...\"` 括起来，以避免文件夹名字中包含空格导致创建文件夹错误的情况。
...
命令格式：
```shell
for ((expression; condition; expression))   # (( ... )) 中的变量与运算符之间添不添加空格均可，且其中的变量使用可以不加 $
do
    语句 1
    语句 2
done
...
> 注意：
>
> 为了防止代码错误导致递归死循环，可以如以上示例代码所示，在进入每一层递归时使用 read 命令实现类似断点作用：
> ```shell
> func() {    # 递归计算 $1 + ($1 - 1) + ($1 - 2) + ... + 0
>     word=""
>     while [ "${word}" != 'y' ] && [ "${word}" != 'n' ]
...
> ```
>
> 如以上代码段所示，通过 `read` 命令实现在进入每一层递归时都需要手动输入信息，因此可以避免意料之外的递归死循环。
>
> 在实际开发或调试过程中，可以通过 `read` 命令在进入每一层递归时手动输入记录信息，以达到记录调试日志的目的，便于代码调试。
...
习题 1：
> 编写脚本 `main.sh`，要求：
> * 在 stdout（标准输出）中输出 1 ~ n 的按字典序从小到大的顺序数第 m 个全排列，输出一行，用空格隔开所有数，行末可以有多余空格。
> * 数据保证 `1 <= n <= 100`、`1 <= m <= min(100, n!)`，脚本不需要判断数据的合法性。

实现思路：
> 若 n = 3，则按字典序从小到大的顺序输出所有全排列为：
> ```shell
> 1. 1 2 3
> 2. 1 3 2
> 3. 2 1 3
...
> ```
>
> 解本题需要使用算法基础课的 level 2 中的 DFS 全排列算法，因此本题暂时略过。
